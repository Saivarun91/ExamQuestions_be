...
def get_all_coupons(request):
    """
    Admin API: Get all coupons created by admin (source coupons only).
    Excludes user-specific coupon copies that are automatically created when sending coupons to users.
    """
    if request.method != "GET":
        return JsonResponse({"success": False, "message": "Method not allowed"}, status=405)

    try:
        # Only show source coupons created by admin - exclude user-specific copies
        # Source coupons: user=None (not assigned to specific user), or is_common=True
        # Exclude: user-specific copies (user is not None and is_common=False)
        coupons = Coupon.objects(
            user=None  # Only source coupons, not user-specific copies
        ).order_by('-created_at')
        ...
        try:
            ...
            if "duplicate key" in error_str.lower() or "E11000" in error_str:
                ...
                if existing:
                    ...
                    # Use the exact code - don't add suffix
                    # The model allows same code for multiple leads, so just retry with same code
                    try:
                        lead_coupon = Coupon(
                            code=source_coupon.code,  # Use exact code, no suffix
                            lead=lead,
                            discount_type=source_coupon.discount_type,
                            discount_value=source_coupon.discount_value,
                            min_purchase=source_coupon.min_purchase,
                        ...
                        # If still fails, return error - don't modify the code
                        print(f"Error creating coupon with exact code: {retry_error}")
                        return JsonResponse({
                            "success": False,
                            "message": f"Failed to create coupon. Please ensure MongoDB index allows duplicate codes for different leads."
                        }, status=500)
            else:
...
def assign_coupon_to_user(request, user_id):
    ...
    try:
        ...
        # Note: MongoDB has a unique index on 'code' field
        # This means only ONE coupon with a given code can exist in the collection
        # Since source coupon already exists with this code, we cannot create a user-specific copy
        # Instead, we'll use the source coupon and track usage via 'used_by' list
        # OR we can check if we can create it and handle the NotUniqueError if it fails
        ...
        # Note: MongoDB may have a unique index on 'code' field, so we need to handle NotUniqueError
        user_coupon = None
        try:
            user_coupon = Coupon(
            ...
            # Handle NotUniqueError and duplicate key errors (MongoDB index on code field)
            from mongoengine.errors import NotUniqueError
            
            error_str = str(save_error)
            is_duplicate_error = (
                isinstance(save_error, NotUniqueError) or 
                "duplicate key" in error_str.lower() or 
                "E11000" in error_str or
                "NotUniqueError" in error_str
            )
            ...
            if is_duplicate_error:
                ...
                if existing:
                    # User already has this coupon, use the existing one
                    user_coupon = existing
                    print(f"✓ Coupon already exists for user, using existing: {existing.code}")
                else:
                    # Duplicate key error but coupon doesn't exist for this user
                    # This means MongoDB has a unique index on 'code' field
                    # Check if there's any coupon with this code (could be source coupon or another user's)
                    any_coupon_with_code = Coupon.objects(code=source_coupon.code).first()
                    if any_coupon_with_code and any_coupon_with_code.user == user:
                        # Found it - use it
                        user_coupon = any_coupon_with_code
                        print(f"✓ Found existing coupon for user: {user_coupon.code}")
                    else:
                        # MongoDB index prevents creating coupon with same code
                        # Since we can't create it, check one more time if it was created by another process
                        import time
                        time.sleep(0.1)  # Small delay to check for race condition
                        existing_check = Coupon.objects(code=source_coupon.code, user=user).first()
                        if existing_check:
                            user_coupon = existing_check
                            print(f"✓ Coupon found after retry: {user_coupon.code}")
                        else:
                            # Can't create due to MongoDB unique index on code field
                            # Since source coupon exists, we'll use it directly for the user
                            # The source coupon is common (user=None), so we can track usage via 'used_by'
                            print(f"⚠️ MongoDB unique index prevents creating user-specific copy")
                            print(f"  Using source coupon '{source_coupon.code}' directly for user")
                            user_coupon = source_coupon
                            # Note: The source coupon will be shared, but usage is tracked per user via 'used_by' list
            else:
                # Re-raise if it's a different error
                print(f"Error saving coupon: {save_error}")
                import traceback
                print(traceback.format_exc())
                raise
        
        # Ensure user_coupon was created successfully
        if not user_coupon:
            return JsonResponse({
                "success": False,
                "message": "Failed to create coupon for user"
            }, status=500)
        ...
        # Send email to user using email template
        email_sent = False
        try:
            from enrollments.email_utils import send_coupon_email
            if user.email:
                user_name = user.fullname if hasattr(user, 'fullname') and user.fullname else (user.name if hasattr(user, 'name') and user.name else "Student")
                email_sent = send_coupon_email(
                    ...
                    min_purchase=user_coupon.min_purchase,
                    max_discount=user_coupon.max_discount
                )
                if email_sent:
                    print(f"✓ Coupon email sent successfully to {user.email}")
                else:
                    print(f"⚠️ Coupon email could not be sent to {user.email} - check email template configuration")
        except Exception as e:
            print(f"✗ Error sending coupon email to {user.email}: {e}")
            import traceback
            print(traceback.format_exc())
            # Don't fail the coupon assignment if email fails, but log the error
        ...
        for user in all_users:
            try:
                ...
                try:
                    ...
                    if "duplicate key" in error_str.lower() or "E11000" in error_str:
                        ...
                        if existing:
                            ...
                            # Use the exact code - don't add suffix
                            # The model allows same code for multiple users, so just retry with same code
                            try:
                                user_coupon = Coupon(
                                    code=source_coupon.code,  # Use exact code, no suffix
                                    user=user,
                                ...
                                # If still fails, skip this user and log error
                                print(f"Error creating coupon with exact code for user {user.id}: {retry_error}")
                                error_count += 1
                                continue
                    else:
                        # Re-raise if it's a different error
                ...
                try:
                    ...
                    print(f"✗ Error sending coupon email to user {user.id} ({user.email if hasattr(user, 'email') else 'no email'}): {e}")
                    import traceback
                    print(traceback.format_exc())
                    # Don't fail the coupon assignment if email fails, but log the error

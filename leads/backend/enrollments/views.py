        ...
        if course_id:
            ...
            if user:
                ...
                if not category_already_enrolled:
                    ...
                    from categories.models import TestCategory
                    category_doc = TestCategory.objects(id=category.id).first()
                    if category_doc:
...
def check_enrollment(request, category_id):
    ...
    try:
        ...
        if course:
            # Check enrollment by course
            enrollment = Enrollment.objects(user_name=userId, course=course).first()
            enrolled = enrollment is not None
            # Also check in user's enrolled_courses list
            if not enrolled:
                enrolled = any(str(c.id) == str(course.id) for c in user.enrolled_courses if hasattr(c, 'id'))
        else:
            # Check enrollment by category (backward compatibility)
            from categories.models import TestCategory
            category = TestCategory.objects(id=ObjectId(category_id)).first()
            if category:
                enrollment = Enrollment.objects(user_name=userId, category=category).first()
                enrolled = enrollment is not None
                # Also check in user's enrolled_courses list
                if not enrolled:

...
def get_recently_updated_exams(request):
    ...
    try:
        ...
        for exam in exams:
            ...
            # Serialize practice_tests_list for calculating total questions
            practice_tests_list = []
            try:
                # Get practice_tests from reference field
                practice_tests = getattr(exam, 'practice_tests', [])
                seen_ids = set()
...
def subscribe_email(request):
    ...
    try:
        ...
        existing_subscriber = EmailSubscriber.objects(email=email).first()

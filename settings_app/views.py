...
def update_admin_settings(request):
    ...
    try:
        ...
        if "social_facebook_url" in body:
            settings_obj.social_facebook_url = body.get("social_facebook_url", "")
        if "social_twitter_url" in body:
            settings_obj.social_twitter_url = body.get("social_twitter_url", "")
        if "social_linkedin_url" in body:
            settings_obj.social_linkedin_url = body.get("social_linkedin_url", "")
        if "social_youtube_url" in body:
            settings_obj.social_youtube_url = body.get("social_youtube_url", "")
        if "social_instagram_url" in body:
            settings_obj.social_instagram_url = body.get("social_instagram_url", "")
        
        # Update font settings if provided
        if "font_family" in body:
            font_family = body.get("font_family", "Poppins")
            # Accept any font name (from dropdown or manually entered)
            # Just clean it up (trim whitespace)
            if font_family:
                font_family = str(font_family).strip()
                settings_obj.font_family = font_family if font_family else "Poppins"
            else:
                settings_obj.font_family = "Poppins"
        if "font_size" in body:
            font_size = body.get("font_size", "16")
            # Validate font size (between 10 and 24)
            try:
                size = int(font_size)
                if 10 <= size <= 24:
                    settings_obj.font_size = str(size)
                else:
...
def get_privacy_policy(request):
    ...
    try:
        ...
        # Safely format updated_at
        updated_at_str = None
        if policy.updated_at:
            try:
                if hasattr(policy.updated_at, 'isoformat'):
                    updated_at_str = policy.updated_at.isoformat()
                else:
                    updated_at_str = str(policy.updated_at)
            except Exception:
                updated_at_str = None
        
        return JsonResponse({
            "success": True,
            "content": policy.content or "Privacy policy content will be updated by admin.",
...
def get_terms_of_service(request):
    ...
    try:
        ...
        if terms.updated_at:
            try:
                if hasattr(terms.updated_at, 'isoformat'):
                    updated_at_str = terms.updated_at.isoformat()
                else:
                    updated_at_str = str(terms.updated_at)
            except Exception:
                updated_at_str = None
